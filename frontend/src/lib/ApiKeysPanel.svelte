...
<section class="keys">
  <header class="intro">
    <h2>API keys</h2>
    <p class="lead">
      Grant a third-party app access to your data over the REST API. A key acts
      as <strong>you</strong>, limited to the scopes you choose. Send it as
  ...
  {#if revealed}
    <div class="reveal" role="status">
      ...
      <div class="token-row">
        <input class="token" readonly value={revealed.token} onclick={(e) => e.currentTarget.select()} />
  ...
  <form class="card create" onsubmit={create}>
    ...
    <div class="actions">
      <button type="submit" class="primary" disabled={busy}>{busy ? "Creating…" : "Create key"}</button>
    </div>
  </form>
    ...
    <div class="table">
      ...
      {#each keys as k (k.id)}
        <div class="trow" role="row" class:inactive={status(k) !== "Active"}>
          ...
          <span data-label="Status"><span class="badge {status(k).toLowerCase()}">{status(k)}</span></span>
...
<style>
  .keys { display: flex; flex-direction: column; gap: 1.25rem; animation: rise var(--t-slow) var(--ease-out) both; }
  .intro h2 { margin: 0; font-size: 1.4rem; font-weight: 800; letter-spacing: -0.02em; }
  .lead { margin: 0.4rem 0 0; color: var(--muted); line-height: 1.55; max-width: 60ch; }
  code { font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 0.85em; background: var(--surface); padding: 0.08rem 0.35rem; border-radius: var(--r-xs); border: 1px solid var(--border); }

  .card {
    background: var(--bg-elevated); border: 1px solid var(--border); border-radius: var(--r-lg);
    padding: 1.25rem; display: flex; flex-direction: column; gap: 0.9rem; box-shadow: var(--shadow-md);
  }
  ...
  .scopes { border: 1px solid var(--border); border-radius: var(--r-md); padding: 0.75rem; background: var(--surface); display: flex; flex-wrap: wrap; gap: 0.5rem; }
  legend { font-weight: 700; font-size: 0.85rem; padding: 0 0.4rem; }
  .chip {
    flex-direction: row; align-items: center; gap: 0.4rem; cursor: pointer; user-select: none;
    padding: 0.35rem 0.7rem; border: 1px solid var(--border); border-radius: var(--r-full);
    background: var(--bg-elevated); font-weight: 600; font-size: 0.85rem;
    transition: background var(--t-fast) var(--ease), border-color var(--t-fast) var(--ease);
  }
  .chip.on { background: var(--accent-soft); border-color: color-mix(in oklab, var(--accent) 45%, transparent); color: var(--accent); }
  .chip input { width: auto; accent-color: var(--accent); }
  .chip-note { font-weight: 400; color: var(--muted); font-size: 0.78rem; }
  ...
  button.primary { background: var(--accent); border-color: var(--accent); color: var(--on-accent); box-shadow: var(--shadow-sm); }
  button.primary:hover { background: var(--accent-hover); border-color: var(--accent-hover); }
  button.danger { color: var(--danger); border-color: color-mix(in oklab, var(--danger) 45%, var(--border)); font-size: 0.82rem; padding: 0.35rem 0.7rem; }
  button.danger:hover { background: var(--danger-soft); border-color: var(--danger); }
  button.ghost { border: none; background: transparent; color: inherit; text-decoration: underline; text-underline-offset: 2px; padding: 0.2rem 0.4rem; align-self: flex-start; }
  ...
  .reveal {
    ...
    border-radius: var(--r-lg); padding: 1rem 1.15rem; display: flex; flex-direction: column; gap: 0.7rem;
    box-shadow: var(--shadow-sm);
  }
  .reveal-head { display: flex; flex-direction: column; gap: 0.15rem; }
  .reveal-head strong { font-weight: 700; }
  .warn { font-size: 0.82rem; color: var(--danger-text); font-weight: 600; }
  .token-row { display: flex; gap: 0.5rem; }
  .token { flex: 1; min-width: 0; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 0.85rem; background: var(--bg); }
  ...
  .empty { border: 1px dashed var(--border); border-radius: var(--r-lg); background: var(--surface); padding: 1.5rem; text-align: center; }

  .table { display: flex; flex-direction: column; border: 1px solid var(--border); border-radius: var(--r-lg); overflow: hidden; background: var(--bg-elevated); box-shadow: var(--shadow-sm); }
  .thead, .trow { display: grid; grid-template-columns: 1.4fr 1.2fr 1.1fr 0.9fr 1fr 1fr auto; gap: 0.6rem; align-items: center; padding: 0.65rem 0.9rem; }
  ...
  .trow { border-top: 1px solid var(--border); font-size: 0.87rem; }
  .trow.inactive { opacity: 0.6; }
  ...
  .pill { font-size: 0.72rem; padding: 0.08rem 0.5rem; border: 1px solid var(--border); border-radius: var(--r-full); margin-right: 0.25rem; background: var(--surface); }
  .badge { font-size: 0.72rem; font-weight: 700; padding: 0.1rem 0.5rem; border-radius: var(--r-full); }
  .badge.active { color: var(--success-text); background: color-mix(in oklab, var(--success) 15%, transparent); }
  .badge.revoked { color: var(--danger-text); background: var(--danger-soft); }
  .badge.expired { color: var(--muted); background: var(--surface); }
