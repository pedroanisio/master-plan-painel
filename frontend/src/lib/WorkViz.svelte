<script lang="ts">
  ...
  const kpis = $derived.by(() => {
    ...
    return {
      total, active, span: days.length, longest, last7, prev7, wow,
  ...
  const ma = $derived.by(() => {
    const w = 7, out: number[] = [];
    for (let i = 0; i < series.length; i++) {
      let sum = 0, n = 0;
      for (let j = Math.max(0, i - w + 1); j <= i; j++) { sum += series[j].minutes; n++; }
      out.push(n ? sum / n : 0);
    }
    return out;
  });
  ...
  const KIND_SLOT: Record<string, number> = {
    feature: 1, research: 2, review: 3, docs: 4, test: 5, infra: 6, refactor: 7, bugfix: 8,
  };
  ...
  const PROJ_WINDOWS: { key: ProjWindow; label: string }[] = [
    { key: 7, label: "7d" }, { key: 14, label: "14d" }, { key: 30, label: "30d" }, { key: 0, label: "All" },
  ];
  const TOP_N = 12;
  const byProject = $derived.by(() => {
    const cutoff = projWindow === 0
      ? -Infinity
      : startOfToday().getTime() - (projWindow - 1) * 86_400_000;
    const m = new Map<string, number>();
    for (const e of entries) {
...
<section class="viz">
  ...
  <div class="kpis">
    ...
    <div class="kpi">
      ...
      <span class="k-sub">{kpis.span ? Math.round((kpis.active / kpis.span) * 100) : 0}% of span</span>
    </div>
    <div class="kpi">
    ...
    <div class="kpi">
      ...
      <span class="k-sub trend-{kpis.wow >= 0 ? 'up' : 'down'}">
        {kpis.wow >= 0 ? "▲" : "▼"} {Math.abs(Math.round(kpis.wow * 100))}% vs prior 7
      </span>
    </div>
  </div>
  ...
  {:else if mode === "trend"}
    ...
    >
      <svg viewBox={`0 0 ${Math.max(series.length, 1)} 100`} preserveAspectRatio="none" class="plot">
        {#each series as d, i (d.key)}
          <rect
            class="bar" class:on={hover === i}
            x={i + 0.12} width="0.76"
            y={100 - (d.minutes / seriesMax) * 100}
            height={(d.minutes / seriesMax) * 100}
          ></rect>
        {/each}
        <polyline class="ma" points={maPoints} vector-effect="non-scaling-stroke" fill="none"></polyline>
      </svg>
      {#if hover !== null}
  ...
  {:else}
    ...
    <div class="sub-head">
      <span class="caption">
        {#if mode === "project"}
          Top projects · {projWindow === 0 ? "all time" : `last ${projWindow} days`} · {byProject.count} active
      ...
      {#if mode === "project"}
        <div class="ranges">
          {#each PROJ_WINDOWS as w (w.key)}
            <button class:active={projWindow === w.key} onclick={() => (projWindow = w.key)}>{w.label}</button>
          {/each}
        </div>
      {/if}
    </div>
      ...
      {:else}
        <div class="cat">
          ...
          {#if byProject.others > 0}
            {@render catRow(`+ ${byProject.count - TOP_N} more`, byProject.others, "var(--muted)", false, byProject.windowTotal)}
          {/if}
        </div>
      {/if}
    {:else if mode === "kind"}
...
{#snippet catRow(name: string, mins: number, color: string, swatch: boolean, denom: number)}
  <div class="cat-row">
    ...
    <span class="cat-track">
      <span class="cat-fill" style={`width:${(mins / (denom || 1)) * 100}%; background:${color}`}></span>
    </span>
    <span class="cat-val">{formatMinutes(mins)} <span class="cat-pct">{((mins / (denom || 1)) * 100).toFixed(0)}%</span></span>
  </div>
{/snippet}
...
<style>
  .viz {
    ...
    --k1: #2a78d6; --k2: #008300; --k3: #e87ba4; --k4: #eda100;
    --k5: #1baf7a; --k6: #eb6834; --k7: #4a3aa7; --k8: #e34948;
    background: var(--bg-elevated); border: 1px solid var(--border); border-radius: var(--r-lg);
    padding: 1.2rem 1.35rem; display: flex; flex-direction: column; gap: 1rem;
    box-shadow: var(--shadow-md); animation: rise var(--t-slow) var(--ease-out) both;
  }
  @media (prefers-color-scheme: dark) {
    .viz { --k1: #3987e5; --k2: #008300; --k3: #d55181; --k4: #c98500;
           --k5: #199e70; --k6: #d95926; --k7: #9085e9; --k8: #e66767; }
  }
  ...
  .kpi {
    background: var(--surface); padding: 0.75rem 0.9rem; display: flex; flex-direction: column; gap: 0.15rem;
    transition: background var(--t-fast) var(--ease);
  }
  .kpi:hover { background: var(--surface-hover); }
  .k-label { font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.06em; color: var(--muted); font-weight: 600; }
  .k-value { font-size: 1.5rem; font-weight: 700; font-variant-numeric: tabular-nums; line-height: 1.1; }
  .k-value small { font-size: 0.6em; color: var(--muted); font-weight: 600; margin-left: 1px; }
  .k-sub { font-size: 0.74rem; color: var(--muted); }
  .k-sub.trend-up { color: var(--success-text); font-weight: 600; }
  .k-sub.trend-down { color: var(--danger); font-weight: 600; }

  .head { display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }
  .modes, .ranges { display: flex; gap: 0.25rem; flex-wrap: wrap; }
  .modes button, .ranges button {
  ...
  .modes button:hover, .ranges button:hover { color: var(--text); background: var(--surface-hover); border-color: var(--border-strong); }
  .modes button:active, .ranges button:active { transform: translateY(1px); }
  .modes button.active, .ranges button.active { color: var(--on-accent); background: var(--accent); border-color: var(--accent); box-shadow: var(--shadow-sm); }
  .modes button.active:hover, .ranges button.active:hover { background: var(--accent-hover); border-color: var(--accent-hover); color: var(--on-accent); }
  .muted { color: var(--muted); margin: 0; }
  .caption { font-size: 0.82rem; color: var(--muted); }
  .ma-key { color: var(--text); border-bottom: 2px solid var(--accent); padding-bottom: 1px; }
  ...
  .trend { position: relative; height: 180px; width: 100%; touch-action: none; }
  .plot { width: 100%; height: 100%; display: block; overflow: visible; }
  .bar { fill: color-mix(in oklab, var(--accent) 42%, transparent); transition: fill var(--t-fast) var(--ease); }
  .bar.on { fill: var(--accent); }
  .ma { stroke: var(--accent); stroke-width: 2; stroke-linejoin: round; opacity: 0.95; }
  @media (prefers-color-scheme: dark) { .ma { stroke: var(--text); opacity: 0.85; } }
  .crosshair { position: absolute; top: 0; bottom: 0; width: 1px; background: var(--muted); transform: translateX(-0.5px); pointer-events: none; }
  .dot { position: absolute; width: 8px; height: 8px; border-radius: 50%; background: var(--accent);
    border: 2px solid var(--bg-elevated); transform: translate(-50%, -50%); pointer-events: none;
    box-shadow: 0 0 0 4px color-mix(in oklab, var(--accent) 22%, transparent); }
  .tt {
    position: absolute; top: -0.4rem; transform: translate(-50%, -100%);
    background: var(--text); color: var(--bg-elevated); border-radius: var(--r-sm); padding: 0.4rem 0.65rem;
    display: flex; flex-direction: column; gap: 1px; white-space: nowrap; pointer-events: none;
    font-size: 0.76rem; box-shadow: var(--shadow-lg); z-index: 3;
    animation: fade var(--t-fast) var(--ease-out) both;
  }
  .tt strong { font-size: 0.8rem; }
  .tt .muted { color: color-mix(in oklab, var(--bg) 65%, var(--text)); }
  .tt.flip { transform: translate(-50%, -100%); }
  .axis { display: flex; justify-content: space-between; font-size: 0.75rem; color: var(--muted); font-variant-numeric: tabular-nums; }

  /* Weekday cadence */
  .weekbars { display: grid; grid-template-columns: repeat(7, 1fr); gap: 0.5rem; height: 160px; align-items: end; }
  .wcol { display: flex; flex-direction: column; align-items: center; justify-content: flex-end; height: 100%; gap: 0.3rem; }
  .wbar {
    width: 62%; min-height: 2px; border-radius: 4px 4px 0 0;
  ...
  .wcol:hover .wbar { filter: brightness(1.12) saturate(1.1); transform: scaleY(1.03); }
  .wlabel { font-size: 0.72rem; color: var(--muted); font-weight: 600; }
  .wval { font-size: 0.72rem; color: var(--text); font-variant-numeric: tabular-nums; }

  /* Calendar heatmap */
  .heatmap { display: grid; grid-template-rows: repeat(7, 1fr); grid-auto-flow: column; grid-auto-columns: 1fr; gap: 3px; overflow-x: auto; }
  .cell {
    width: 100%; aspect-ratio: 1; border-radius: 3px; min-width: 10px;
    transition: transform var(--t-fast) var(--ease-spring), outline-color var(--t-fast) var(--ease);
    outline: 1px solid transparent; outline-offset: -1px;
  }
  .heatmap .cell:hover { transform: scale(1.35); outline-color: var(--accent); position: relative; z-index: 1; }
  .cell.lvl-0 { background: color-mix(in oklab, var(--border) 45%, transparent); }
  .cell.lvl-1 { background: color-mix(in oklab, var(--accent) 28%, transparent); }
  .cell.lvl-2 { background: color-mix(in oklab, var(--accent) 50%, transparent); }
  .cell.lvl-3 { background: color-mix(in oklab, var(--accent) 74%, transparent); }
  .cell.lvl-4 { background: var(--accent); }
  .legend { display: flex; align-items: center; gap: 0.35rem; font-size: 0.78rem; color: var(--muted); flex-wrap: wrap; }
  .legend .cell { width: 0.85rem; height: 0.85rem; aspect-ratio: auto; }
  .legend-note { margin-left: 0.25rem; }

  /* Categorical bars */
  .cat { display: flex; flex-direction: column; gap: 0.45rem; }
  .cat-row { display: grid; grid-template-columns: minmax(110px, 1.2fr) 3fr auto; gap: 0.75rem; align-items: center; font-size: 0.85rem; }
  .cat-name { font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; display: flex; align-items: center; gap: 0.4rem; }
  .swatch { width: 0.8rem; height: 0.8rem; border-radius: var(--r-xs); border: 1px solid var(--border); flex: none; display: inline-block; }
  .cat-track { background: var(--surface); border: 1px solid var(--border); border-radius: var(--r-full); height: 0.7rem; overflow: hidden; }
  .cat-fill { display: block; height: 100%; min-width: 2px; }
  .cat-val { font-variant-numeric: tabular-nums; color: var(--muted); min-width: 5.5rem; text-align: right; }
  .cat-pct { color: var(--text); font-weight: 600; }

  @media (max-width: 560px) {
    .viz { padding: 0.95rem 0.85rem; }
    .kpis { grid-template-columns: repeat(2, 1fr); }
    .k-value { font-size: 1.3rem; }
    .trend { height: 150px; }
    .weekbars { height: 130px; gap: 0.3rem; }
    .wval { font-size: 0.68rem; }
    /* Tighter bar rows so the value never collides with the label. */
    .cat-row { grid-template-columns: minmax(84px, 1.1fr) 1.8fr auto; gap: 0.5rem; font-size: 0.82rem; }
    .cat-val { min-width: 4.6rem; }
  }
  @media (max-width: 380px) {
    .kpis { grid-template-columns: 1fr; }
    .tt { max-width: 60vw; white-space: normal; }
  }
</style>
